...
See the [command-line instructions](../README.md#try-it) for
...
- **Model contract and runtime path.** [`ArchConfig`](../Sources/Mference/Infrastructure/ModelIO/ModelTypes.swift)
  defines the fixed Gemma 4 shape; [`RuntimeConfiguration`](../Sources/Mference/Runtime/Configuration/RuntimeConfiguration.swift)
  defines the production configuration.
- **Remote install and `.gturbo` layout.** Start with
  [`SupportedModelSource`](../Sources/MferenceRepack/Core/Remote/SupportedModelSource.swift),
  [`RemoteStreamingRepacker`](../Sources/MferenceRepack/Core/Remote/RemoteStreamingRepacker.swift),
  and [`RepackPlanner`](../Sources/MferenceRepack/Core/Planning/RepackPlanner.swift)
  for the pinned source, bounded range repack, and resident/per-layer file plan.
- **Integrity and model load.** [`ManifestReader`](../Sources/Mference/Infrastructure/ModelIO/ManifestReader.swift),
  [`VerifiedInstallReceipt`](../Sources/Mference/Infrastructure/ModelIO/VerifiedInstallReceipt.swift),
  and [`Model.load`](../Sources/Mference/Runtime/Inference/Model.swift) cover
  validation, resident mapping, and lazy layer verification.
- **Resident and streamed weights.** [`ResidentBuffer`](../Sources/Mference/Infrastructure/ModelIO/ResidentBuffer.swift),
  [`ModelExpertIO`](../Sources/Mference/Runtime/Inference/ModelExpertIO.swift),
  and [`PreadExpertStreamer`](../Sources/Mference/Infrastructure/Streaming/PreadExpertStreamer.swift)
  own common weights, expert-cache planning, slots, and parallel bounded reads.
- **KV cache and attention.** [`KVCacheManager`](../Sources/Mference/Runtime/KVCache/KVCacheManager.swift)
  owns bounded circular SWA storage and linear full-attention storage.
  [`Attention`](../Sources/Mference/Kernels/Attention/Attention.swift) and
  [`PrefillAttention`](../Sources/Mference/Kernels/Attention/PrefillAttention.swift)
  consume distinct FP16 K/V ranges.
- **Prompt and decode orchestration.** [`runRawCompletion`](../Sources/Mference/Runtime/Generation/RawCompletion.swift)
  owns the outer generation loop; [`RealForwardRunner`](../Sources/Mference/Runtime/Inference/RealForwardRunner.swift)
  owns the per-layer prefill and decode graph.
- **Prefill memory and scheduling.** [`PrefillChunkScratch`](../Sources/Mference/Runtime/Prefill/PrefillChunkScratch.swift),
  [`PrefillRoutedTileScheduler`](../Sources/Mference/Kernels/Prefill/MoE/PrefillRoutedTileScheduler.swift),
  and [`MPPPrefillInt4QMM`](../Sources/Mference/Kernels/TensorCore/MPPPrefillInt4QMM.swift)
  show bounded scratch, slot-safe expert tiles, and staged affine MPP projections.
- **Router and routed MoE.** [`MoE`](../Sources/Mference/Kernels/MoE/MoE.swift)
  and [`moe.metal`](../Sources/Mference/Metal/MoE/moe.metal) implement
  top-8 selection, cached-hit work, affine GeGLU, and weighted down reduction.
- **Metal library and fusions.** [`MetalContext`](../Sources/Mference/Infrastructure/Metal/MetalContext.swift),
  [`tensorops.metal`](../Sources/Mference/Metal/TensorCore/tensorops.metal),
  and [`fused.metal`](../Sources/Mference/Metal/Fusions/fused.metal) show
...
- [The experiments that shaped Mference](OPTIMIZATION_JOURNEY.md)
