...
struct TokenizerLoadCoordinatorTests {
    ...
    func concurrentLoadsShareLoadedTokenizer() async throws {
        let tokenizers = try await withThrowingTaskGroup(of: MFTokenizer.self) { group in
            for _ in 0..<8 {
                group.addTask {
                    try await MFTokenizer.load()
                }
            }

            var loaded: [MFTokenizer] = []
    ...
    func consecutiveLoadsReuseCompletedTask() async throws {
        let first = try await MFTokenizer.load()
        let second = try await MFTokenizer.load()
    ...
    func modelTokenizerSidecarIsDiscovered() throws {
        ...
        let resolved = MFTokenizer.tokenizerFolder(forModelDirectory: model)
