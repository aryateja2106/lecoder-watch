...
| `install/payload/meshd/` | 25 | ~7,500 | The daemon (bun + TypeScript). The ONE shipping copy; server.ts is the route table |
...
| file | size | purpose | checks |
|---|---|---|---|
| `mesh` | XL | command-line control for a LeSearch AI fleet (the meshd HTTP API) | check-apps-ota, check-brand, check-bridge-kill-scope, check-clean-install, check-cross-host-cp, check-fleet-map, check-kb-federation, check-mesh-apps, check-mesh-doctor, check-mesh-hooks, check-mesh-onboarding, check-mesh-skills, check-mesh-uninstall, check-mesh-upgrade, check-mesh-version, check-pair-auth, check-published, check-remote-agent-loop, check-session-snapshots, check-tmux-restart, check-token-rotate |
