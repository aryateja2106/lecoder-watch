# Contracts

*Generated by `python3 scripts/codemap-index.py`; do not edit. The three boundaries that two sides must agree on.*

## Daemon routes

`auth = none` is dispatched before the bearer gate in `server.ts`. Everything else needs `Authorization: Bearer` (or loopback). Route regexes are shown as `/agents/:x/panes`.

| method | path | auth | where |
|---|---|---|---|
| GET | `/a/*` | none | server.ts:1271 |
| ANY | `/agents` | token | server.ts:1330 |
| DELETE | `/agents/:x` | token | server.ts:1395 |
| GET | `/agents/:x/chat` | token | server.ts:1366 |
| POST | `/agents/:x/handoff` | token | handoff.ts:282 |
| GET | `/agents/:x/output` | token | server.ts:1373 |
| GET | `/agents/:x/panes` | token | server.ts:1353 |
| POST | `/agents/:x/panes` | token | server.ts:1357 |
| DELETE | `/agents/:x/panes/:x` | token | server.ts:1390 |
| GET | `/agents/:x/resumable` | token | handoff.ts:276 |
| POST | `/agents/:x/send` | token | server.ts:1384 |
| POST | `/agents/new` | token | server.ts:1399 |
| GET | `/apps` | token | input.ts:527 |
| POST | `/apps` | token | input.ts:531 |
| GET | `/built-apps` | token | apps.ts:242 |
| POST | `/built-apps/:x/install` | token | apps.ts:256 |
| POST | `/built-apps/:x/push` | token | apps.ts:246 |
| GET | `/clipboard` | token | input.ts:480 |
| POST | `/clipboard` | token | input.ts:484 |
| GET | `/desktop` | token | input.ts:493 |
| GET | `/displays` | token | input.ts:498 |
| GET | `/doctor` | token | doctor.ts:121 |
| POST | `/doctor/fix` | token | doctor.ts:122 |
| GET | `/events` | token | server.ts:1332 |
| POST | `/events` | token | server.ts:1333 |
| GET | `/exposures` | token | redact.ts:264 |
| POST | `/exposures/:x` | token | redact.ts:274 |
| POST | `/exposures/record` | token | redact.ts:265 |
| GET | `/files` | token | files.ts:91 |
| GET | `/fs` | token | files.ts:96 |
| POST | `/fs/mkdir` | token | files.ts:105 |
| POST | `/fs/move` | token | files.ts:111 |
| GET | `/fs/read` | token | files.ts:100 |
| ANY | `/health` | none | server.ts:1259 |
| GET | `/input` | token | input.ts:471 |
| POST | `/input` | token | input.ts:474 |
| POST | `/kb` | token | server.ts:1334 |
| PUT | `/kb` | token | server.ts:1334 |
| GET | `/kb/:x/:x` | token | server.ts:1348 |
| GET | `/kb/search` | token | server.ts:1338 |
| POST | `/la/token` | token | push.ts:458 |
| POST | `/open` | token | input.ts:516 |
| POST | `/pair/claim` | none | pair.ts:113 |
| GET | `/pair/new` | none | pair.ts:100 |
| GET | `/push` | token | push.ts:441 |
| POST | `/push/register` | token | push.ts:444 |
| POST | `/push/test` | token | push.ts:494 |
| GET | `/resumable` | token | handoff.ts:270 |
| GET | `/screen.jpg` | token | input.ts:504 |
| ANY | `/stats` | token | server.ts:1328 |
| POST | `/system` | token | input.ts:536 |
| ANY | `/tailnet` | token | server.ts:1329 |
| ANY | `/usage` | token | server.ts:1331 |
| GET | `/volume` | token | input.ts:541 |
| POST | `/volume` | token | input.ts:541 |
| POST | `/wake` | token | server.ts:1318 |

### `meshd/cmux-bridge.ts` (its own listener)

| method | path | auth | where |
|---|---|---|---|
| POST | `/cmux` | see file | meshd/cmux-bridge.ts:47 |
| ANY | `/health` | see file | meshd/cmux-bridge.ts:45 |

### `rmux-bridge/src/server.ts` (its own listener)

| method | path | auth | where |
|---|---|---|---|
| ANY | `/` | see file | rmux-bridge/src/server.ts:444 |
| ANY | `/attach` | see file | rmux-bridge/src/server.ts:456 |
| ANY | `/health` | see file | rmux-bridge/src/server.ts:437 |
| ANY | `/xterm/addon-fit.js` | see file | rmux-bridge/src/server.ts:453 |
| ANY | `/xterm/xterm.css` | see file | rmux-bridge/src/server.ts:447 |
| ANY | `/xterm/xterm.js` | see file | rmux-bridge/src/server.ts:450 |

## Capabilities

The daemon advertises these strings on `/health`; the phone and watch gate features on `supports("x")`. A capability that is advertised but never gated is dead on the client; one gated but not in `DaemonCapabilities.expected` hides silently when the daemon is old.

| capability | gated at | in `expected` |
|---|---|---|
| `events` | — (nothing gates it) | no |
| `newPane` | — (nothing gates it) | no |
| `paneTarget` | — (nothing gates it) | no |
| `usage` | — (nothing gates it) | no |
| `agents` | — (nothing gates it) | no |
| `cmux` | — (nothing gates it) | no |
| `herdr` | — (nothing gates it) | no |
| `tailscale` | — (nothing gates it) | no |
| `kb` | — (nothing gates it) | no |
| `screenPeek` | RemoteScreenView.swift:109 | no |
| `input` | WatchViews.swift:323, WatchViews.swift:802, ContentView.swift:1520, ContentView.swift:1521 | no |
| `files` | — (nothing gates it) | no |
| `push` | — (nothing gates it) | no |
| `pair` | — (nothing gates it) | no |
| `doctor` | — (nothing gates it) | no |
| `wake` | MeshStore.swift:892 | no |
| `screenRegion` | MeshClient.swift:219, RemoteView.swift:322, RemoteScreenView.swift:110 | yes |
| `openUrl` | MeshClient.swift:526 | yes |
| `power` | MeshClient.swift:509, ContentView.swift:572 | yes |
| `laPush` | MeshClient.swift:544 | yes |
| `sessionStatus` | — (nothing gates it) | yes |
| `paste` | MeshClient.swift:300, RemoteScreenView.swift:111 | yes |
| `captureJoin` | MeshClient.swift:280, WatchMeshStore.swift:544 | yes |
| `redact` | ExposedSecretsScreen.swift:43, ExposedSecretsScreen.swift:84 | no |
| `chat` | TerminalView.swift:1200 | no |
| `apps` | TerminalView.swift:113 | no |
| `handoff` | MeshClient.swift:565, MeshClient.swift:574, MeshClient.swift:586 | no |

## Watch → phone relay commands

`enum WatchCommandKind` in `Shared/Models.swift`: every command the watch can send through the phone. Each case must be handled in `iOS/MeshStore.swift` `handle(_:)`; an unhandled case is delivered as a silent tick.

| command | handled in MeshStore |
|---|---|
| `.refresh` | MeshStore.swift:1016 |
| `.agentSend` | MeshStore.swift:1021 |
| `.agentOutput` | MeshStore.swift:1038 |
| `.screenPeek` | MeshStore.swift:1045 |
| `.newAgent` | MeshStore.swift:1060 |
| `.newPane` | MeshStore.swift:1065 |
| `.killAgent` | MeshStore.swift:1026 |
| `.killPane` | MeshStore.swift:1030 |
| `.input` | MeshStore.swift:1070 |
| `.volume` | MeshStore.swift:1075 |
| `.clipboard` | MeshStore.swift:1079 |
| `.system` | MeshStore.swift:1083 |
| `.readClipboard` | MeshStore.swift:1104 |
| `.inputStatus` | MeshStore.swift:1151 |
| `.listApps` | MeshStore.swift:1137 |
| `.activateApp` | MeshStore.swift:1142 |
| `.listDisplays` | MeshStore.swift:1146 |
| `.readPhoneClipboard` | MeshStore.swift:1109 |
| `.openURL` | MeshStore.swift:1120 |
| `.fsList` | MeshStore.swift:1132 |
