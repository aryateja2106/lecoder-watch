...
|---|---|---|
| [getting-started.md](getting-started.md) | Anyone meeting the product for the first time, or writing copy about the first ten minutes | Account → install → pair → first buzz → report a problem, with real screenshots. `scripts/build-web-docs.ts` renders it to `web/getting-started.html`, which lesearch.ai serves, so the page and this doc cannot drift. |
| [onboarding-script.md](onboarding-script.md) | Filming the first tutorial, or answering "what is the fastest way to set this up?" | The ninety-second path — install, `mesh setup`, scan the QR, `mesh hooks install` — with what each command prints, the payoff shot (a live pty in colour, a permission answered from the wrist) and what must not be claimed on camera while the store build is older than the daemon. |
| [product/README.md](product/README.md) | Asking "where does feature X live, and what proves it still works?" | Every implemented feature, per surface, with its file paths and the check that covers it — plus a "not built yet" list so nothing aspirational reads as shipped. |
| [product/design-system.md](product/design-system.md) | Redesigning anything, or deciding where a token or component should live | Tokens with `file:line`, the honest account of where there are none (the Swift surfaces use literals), every reusable view, the screens with their captures, and the interaction rules a check enforces. |
| [publish-plan-2026-09-17.md](publish-plan-2026-09-17.md) | Asking "what is between this tree and invited users?" | 37 ordered tasks with owner, blocking flag, dependency and the proof that closes each; starts from the measured state (TestFlight still serves the 0.5.0 crasher, installer 0.5.2 vs daemon 0.6.0). |
| [review-2026-09-17.md](review-2026-09-17.md) | Before touching the relay, pairing, the daemon's seams, or the plan of record | Architecture 5/10, planning 4/10, release 4/10, security 5/10: 55 findings, every one re-checked by a skeptic against this tree, plus nine the reviewers missed. |
| [adr-2026-09-22-platform-shape.md](adr-2026-09-22-platform-shape.md) | Deciding how onboarding, passwords/VNC, pinned agent tasks, plugins and what-the-phone-reads should work | No password fields (the token is the credential; VNC gets its own generated secret over the daemon); three plugin surfaces (skills, hooks, daemon routes under /x/); artifacts as a folder convention; the order of the next slices. |
| [adr-2026-09-22-on-device-brain.md](adr-2026-09-22-on-device-brain.md) | Deciding what answers "how much Claude is left / what's waiting" on the phone or the wrist without a network hop | Needle 2 as the on-device tool-caller over `MeshStore`'s own functions, Jev daemon-side for triage, llama.cpp-on-watch kept as a recipe; the first slice's done criteria and budgets. |
| [../references/reference-projects.md](../references/reference-projects.md) | Building something a reference project already solved (device pane, watch inference, MoE streaming, on-device task models) | mobilecode, Apple-Watch-Edge-AI, flash-moe, Needle 2, fx, Edge0, Jev, Desert Ant — what to take from each and what to leave. |
| [adr-2026-09-22-local-brains-and-streaming-moe.md](adr-2026-09-22-local-brains-and-streaming-moe.md) | Deciding anything about local models: intent on the watch, a big model on the Jetson/Pi/Mac, or building our own engine | Measured 2026-09-22 on the fleet: Needle 2 as the intent router, edge0/ollama fronted by `/brain`, own streaming-MoE engine deferred past launch with a measured target. |
| [icp-and-monetization.md](icp-and-monetization.md) | Writing copy, picking a price, deciding who to invite first | The customer that fits today (agent-native solo developer with a Watch, not the non-technical user), the 2026 competitor table with prices, and the monetization order: sponsors now, one-time IAP later, consulting as the revenue line. |
...
| [agents/CODEMAP.md](agents/CODEMAP.md) | Finding which file to open, before any grep | Generated from the tree and checked against it: every code file's purpose, size and the checks that name it. Siblings: [CONTRACTS.md](agents/CONTRACTS.md) (routes, capabilities, relay commands), [CHECKS.md](agents/CHECKS.md), [SYMBOLS.md](agents/SYMBOLS.md) (grep only). |
| [agents/UI-MAP.md](agents/UI-MAP.md) | Finding an iPhone or Apple Watch screen, its navigation path, daemon routes and checks | Hand-written screen-level companion to the generated file-level maps, plus shared client state, relay rules and task entry points. |
