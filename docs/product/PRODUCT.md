...
| Screen & control — `RemoteScreenView.swift` | Screen, trackpad gestures, zoom to a region that arrives sharp; frames back-to-back; the edge back-swipe waits for the trackpad. Controls are one floating capsule (pointer/pan, zoom, keyboard, more). **The keyboard is the machine's**: the system keyboard types straight through (no field, no Send), with a bar of sticky modifiers, escapes, arrows and chords above it — ⌘q from the phone closed an app on the Pi (**0.6**) | Tap / drag / zoom / type | `/screen.jpg`, `screenRegion`, `/input` (moveTo is absolute on Linux too) |
