...
| Native terminal — `NativeTerminalScreen.swift` (**0.8**) | The pane as a terminal: SwiftTerm paints colour, cursor and alt-screen; a fixed key bar (sticky Ctrl/Alt, Esc, Tab, ↑, dictate, keyboard) under it; the system keyboard types straight into the pane; pinch sets the font. On a `pty` daemon the pane is attached over a WebSocket (`Shared/PtyClient.swift`): sized to the phone, raw bytes each way, reconnects with backoff. On a `captureAnsi` daemon it repaints from `/output?ansi=1` polls and sends through `/send`; wider panes scroll sideways. Reached from *Open terminal* | Type / dictate | `/agents/:s/pty`, `/output?ansi=1`, `/send` (ctrl-/alt-letter keys), `Shared/TerminalKeyRouter.swift`, SwiftTerm 1.18 (SPM) |
