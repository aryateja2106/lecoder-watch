...
| Native terminal — `NativeTerminalScreen.swift` (**0.8**) | The pane as a terminal: SwiftTerm paints colour, cursor and alt-screen from `/output?ansi=1`; a fixed key bar (sticky Ctrl/Alt, Esc, Tab, ↑, dictate, keyboard) under it; the system keyboard types straight into the pane; pinch sets the font; wider panes scroll sideways. Reached from *Open terminal* on a `captureAnsi` daemon | Type / dictate | `/output?ansi=1`, `/send` (ctrl-/alt-letter keys), `Shared/TerminalKeyRouter.swift`, SwiftTerm 1.18 (SPM) |
