#!/bin/sh
# Spare daemon on 127.0.0.1:8898. An allowed local ask saves one reply
# note (the title is the ask, clipped to 200 characters). A later
# POST /agent-note with no id, a q that matches only that reply, and a
# new allowed local ask sends the first reply, a blank line, and the ask,
# writes one relative file mode 600, does not execute it, and saves the
# second reply as another note. A later POST /agent-note with no id and a
# q that matches only the second reply drafts that reply to one relative
# file, mode 600, and does not execute it. It does not draft the first
# reply or the original note. A q that matches the first reply and the
# second reply is 409 and writes nothing. A pairing-code ask on the
# second hop does not call the model, writes no draft, and adds no note.
# The list stays {id, title}. This script does not use port 8899, a real
# home directory, or a hosted model.
set -eu
...
TH="$(mktemp -d /tmp/second-reply-draft.XXXXXX)"
HOME_DIR="$TH/home"
STATE="$TH/state"
WORK="$TH/session"
LOG="$TH/meshd.log"
...
FIRST_REPLY='The first margin stays beside the sentence.'
SECOND_REPLY='The second margin stays beside the sentence.'
FIRST_ASK="$(python3 -c 'print("build a reader " + ("x" * 240))')"
SECOND_ASK='add a gloss'
ONLY_FIRST='first margin'
ONLY_SECOND='second margin'
BOTH_REPLIES='margin stays beside'
...
echo "check-second-reply-draft: paper note is stored"

MODEL="http://127.0.0.1:${STUB_PORT}/v1"
FIRST_ASK="$FIRST_ASK" python3 - "$TH/ask.json" "$WORK" "$MODEL" "$PAPER_ID" <<'PY'
import json, os, sys
json.dump({
    "id": sys.argv[4],
    "cwd": sys.argv[2],
...
FIRST_ASK="$FIRST_ASK" python3 - "$TH/ask.json.out" "$WORK/held.txt" "$TH/assistant.txt" "$WORK/executed-held" "$STATE" "$PAPER_ID" "$TH/reply.id" <<'PY'
...
cp "$STATE/knowledge/$(cat "$TH/reply.id").json" "$TH/reply-before.json"
...
python3 - "$TH/list.json" "$PAPER_ID" "$TH/reply.id" "$TH/assistant.txt" "$FIRST_ASK" <<'PY'
import json, sys
...
ask = sys.argv[5]
want = ask[:200]
if sentence in raw or "body" in raw or ask in raw:
    raise SystemExit("FAIL: list includes the reply or the full ask")
...
PY
echo "check-second-reply-draft: allowed ask saved one clipped reply note"

printf '%s\n' "$SECOND_REPLY" > "$TH/next.txt"
python3 - "$TH/stub.reply" "$TH/next.txt" <<'PY'
import json, sys
text = open(sys.argv[2]).read()
open(sys.argv[1], "w").write(json.dumps({
    "choices": [{"message": {"role": "assistant", "content": text}}],
...
ONLY_FIRST="$ONLY_FIRST" SECOND_ASK="$SECOND_ASK" python3 - "$TH/loop.json" "$WORK" "$MODEL" <<'PY'
import json, os, sys
body = {
    "q": os.environ["ONLY_FIRST"],
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "follow.txt",
    "command": "touch executed-follow",
    "ask": os.environ["SECOND_ASK"],
}
if "id" in body:
    raise SystemExit("FAIL: reply query included an id")
...
SECOND_ASK="$SECOND_ASK" python3 - "$TH/loop.out" "$WORK/follow.txt" "$STATE/knowledge/$(cat "$TH/reply.id").json" "$WORK/executed-follow" "$TH/stub-bodies/2.json" "$TH/next.txt" "$STATE" "$PAPER_ID" "$TH/reply.id" "$TH/next.id" <<'PY'
import json, os, stat, sys
res_path, draft, note_path, marker, stub_path, assistant, state, paper_id, reply_id_path, next_id_path = sys.argv[1:11]
ask = os.environ["SECOND_ASK"]
raw = open(res_path).read()
data = json.loads(raw)
...
if data.get("draft") != "follow.txt" or os.path.isabs(str(data.get("draft"))):
    raise SystemExit("FAIL: reply draft is %r" % (data.get("draft"),))
reply = note.get("body", "").strip()
title = note.get("title", "").strip()
text = "%s\n\n%s" % (title, reply) if title else reply
want = "%s\n\n%s" % (text, ask)
if not want.endswith("%s\n\n%s" % (reply, ask)):
    raise SystemExit("FAIL: prompt does not end with the saved reply, a blank line, and the ask")
payload = json.load(open(stub_path))
...
if messages[0].get("content") != want:
    raise SystemExit("FAIL: model text is %r" % (messages[0].get("content"),))
fresh = open(assistant).read()
if open(draft).read() != fresh:
    raise SystemExit("FAIL: drafted file is not the new reply")
mode = stat.S_IMODE(os.stat(draft).st_mode)
if mode != 0o600 or mode & 0o111:
    raise SystemExit("FAIL: reply draft mode is %o" % mode)
if os.path.exists(marker):
    raise SystemExit("FAIL: reply draft was executed")
reply_id = open(reply_id_path).read().strip()
names = [name for name in os.listdir(os.path.join(state, "knowledge")) if name.endswith(".json")]
if len(names) != 3:
    raise SystemExit("FAIL: knowledge files are %r" % (names,))
found = None
for name in names:
    saved = json.load(open(os.path.join(state, "knowledge", name)))
    if saved.get("id") in (paper_id, reply_id):
        continue
    if saved.get("title") != ask or saved.get("body") != fresh:
        raise SystemExit("FAIL: new reply note is %r" % (saved,))
    file_mode = stat.S_IMODE(os.stat(os.path.join(state, "knowledge", name)).st_mode)
    if file_mode != 0o600:
        raise SystemExit("FAIL: new reply note mode is %o" % file_mode)
    found = saved["id"]
if not found:
    raise SystemExit("FAIL: new reply note was not written")
open(next_id_path, "w").write(found)
PY
[ "$(stub_hits)" = "$((before_hits + 1))" ] || { echo "FAIL: reply query did not send the ask"; exit 1; }
[ "$(note_count)" = "$((before_notes + 1))" ] || { echo "FAIL: reply query did not save the new reply"; exit 1; }
follow_mode="$(mode_of "$WORK/follow.txt")"
[ "$follow_mode" = "600" ] || { echo "FAIL: follow file mode is $follow_mode, want 600"; exit 1; }
[ ! -e "$WORK/executed-follow" ] || { echo "FAIL: reply query executed a command"; exit 1; }
files_after="$(work_files)"
printf '%s\n' "$files_before" > "$TH/files-before.txt"
printf '%s\n' "$files_after" > "$TH/files-after.txt"
python3 - "$TH/files-before.txt" "$TH/files-after.txt" "$WORK/follow.txt" <<'PY'
import sys
...
cmp -s "$TH/note-before.json" "$STATE/knowledge/${PAPER_ID}.json" || {
  echo "FAIL: second hop changed the paper note"
  exit 1
}
cmp -s "$TH/reply-before.json" "$STATE/knowledge/$(cat "$TH/reply.id").json" || {
  echo "FAIL: second hop changed the first reply note"
  exit 1
}
echo "check-second-reply-draft: query matching only the first reply sent the reply, a blank line, and the ask"

cp "$STATE/knowledge/$(cat "$TH/next.id").json" "$TH/next-before.json"
before_hits="$(stub_hits)"
before_notes="$(note_count)"
files_before="$(work_files)"
ONLY_SECOND="$ONLY_SECOND" python3 - "$TH/second.json" "$WORK" "$MODEL" <<'PY'
import json, os, sys
body = {
    "q": os.environ["ONLY_SECOND"],
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "second.txt",
    "command": "touch executed-second",
}
if "id" in body or "ask" in body:
    raise SystemExit("FAIL: second reply query included an id or an ask")
json.dump(body, open(sys.argv[1], "w"))
PY
code="$(post_json "$TH/second.out" "$TH/second.json" "/agent-note")"
[ "$code" = "200" ] || { echo "FAIL: second reply query -> ${code}"; cat "$TH/second.out"; echo; tail -n 40 "$LOG"; exit 1; }
python3 - "$TH/second.out" "$WORK/second.txt" "$STATE/knowledge/$(cat "$TH/next.id").json" "$WORK/executed-second" "$TH/stub-bodies/3.json" "$TH/assistant.txt" "$TH/note-before.json" <<'PY'
import json, os, stat, sys
res_path, draft, note_path, marker, stub_path, first_path, paper_path = sys.argv[1:8]
raw = open(res_path).read()
data = json.loads(raw)
note = json.load(open(note_path))
...
if data.get("draft") != "second.txt" or os.path.isabs(str(data.get("draft"))):
    raise SystemExit("FAIL: second draft is %r" % (data.get("draft"),))
body = note.get("body", "")
if open(draft).read() != body:
    raise SystemExit("FAIL: drafted file is not the second reply body")
if open(draft).read() == first or body.strip() == first.strip():
    raise SystemExit("FAIL: drafted the first reply")
paper_body = paper.get("body", "")
if paper_body and (paper_body in open(draft).read() or open(draft).read() == paper_body):
    raise SystemExit("FAIL: drafted the original note")
mode = stat.S_IMODE(os.stat(draft).st_mode)
if mode != 0o600 or mode & 0o111:
    raise SystemExit("FAIL: second draft mode is %o" % mode)
if os.path.exists(marker):
    raise SystemExit("FAIL: second draft was executed")
payload = json.load(open(stub_path))
messages = payload.get("messages")
if not isinstance(messages, list) or not messages:
    raise SystemExit("FAIL: second draft messages are %r" % (messages,))
chunks = []
for msg in messages:
    content = msg.get("content")
    chunks.append(content if isinstance(content, str) else json.dumps(content))
sent = "\n".join(chunks)
if body.strip() not in sent:
    raise SystemExit("FAIL: second draft prompt is %r" % (sent,))
if first.strip() in sent:
    raise SystemExit("FAIL: second draft prompt includes the first reply: %r" % (sent,))
paper_text = paper_body.strip()
if paper_text and paper_text in sent:
    raise SystemExit("FAIL: second draft prompt includes the original note: %r" % (sent,))
PY
[ "$(stub_hits)" = "$((before_hits + 1))" ] || { echo "FAIL: second reply query did not draft through the model"; exit 1; }
[ "$(note_count)" = "$before_notes" ] || { echo "FAIL: second reply query added a note"; exit 1; }
second_mode="$(mode_of "$WORK/second.txt")"
[ "$second_mode" = "600" ] || { echo "FAIL: second file mode is $second_mode, want 600"; exit 1; }
[ ! -e "$WORK/executed-second" ] || { echo "FAIL: second reply query executed a command"; exit 1; }
files_after="$(work_files)"
printf '%s\n' "$files_before" > "$TH/files-before-2.txt"
printf '%s\n' "$files_after" > "$TH/files-after-2.txt"
python3 - "$TH/files-before-2.txt" "$TH/files-after-2.txt" "$WORK/second.txt" <<'PY'
import sys
before = [line for line in open(sys.argv[1]).read().splitlines() if line]
after = [line for line in open(sys.argv[2]).read().splitlines() if line]
added = [path for path in after if path not in before]
if added != [sys.argv[3]]:
    raise SystemExit("FAIL: second reply files are %r" % (added,))
PY
cmp -s "$TH/note-before.json" "$STATE/knowledge/${PAPER_ID}.json" || {
  echo "FAIL: drafting the second reply changed the paper note"
  exit 1
}
cmp -s "$TH/reply-before.json" "$STATE/knowledge/$(cat "$TH/reply.id").json" || {
  echo "FAIL: drafting the second reply changed the first reply"
  exit 1
}
cmp -s "$WORK/held.txt" "$TH/assistant.txt" || {
  echo "FAIL: drafting the second reply replaced the first reply file"
  exit 1
}
echo "check-second-reply-draft: query matching only the second reply drafted that reply"

before_hits="$(stub_hits)"
before_notes="$(note_count)"
files_before="$(work_files)"
BOTH_REPLIES="$BOTH_REPLIES" python3 - "$TH/both.json" "$WORK" "$MODEL" <<'PY'
import json, os, sys
body = {
    "q": os.environ["BOTH_REPLIES"],
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "both.txt",
...
if "id" in body:
    raise SystemExit("FAIL: shared query included an id")
json.dump(body, open(sys.argv[1], "w"))
PY
...
echo "check-second-reply-draft: query matching both replies returned 409"

before_hits="$(stub_hits)"
before_notes="$(note_count)"
files_before="$(work_files)"
ONLY_FIRST="$ONLY_FIRST" python3 - "$TH/pair.json" "$WORK" "$MODEL" <<'PY'
import json, os, sys
body = {
    "q": os.environ["ONLY_FIRST"],
    "cwd": sys.argv[2],
...
echo "check-second-reply-draft: pairing-code ask on the second hop wrote nothing"
...
cmp -s "$TH/reply-before.json" "$STATE/knowledge/$(cat "$TH/reply.id").json" || {
  echo "FAIL: drafting changed the first reply"
  exit 1
}
cmp -s "$TH/next-before.json" "$STATE/knowledge/$(cat "$TH/next.id").json" || {
  echo "FAIL: drafting changed the second reply"
  exit 1
}
...
python3 - "$TH/list2.json" "$PAPER_ID" "$TH/reply.id" "$TH/next.id" "$TH/assistant.txt" "$TH/next.txt" "$FIRST_ASK" "$SECOND_ASK" <<'PY'
import json, sys
raw = open(sys.argv[1]).read()
data = json.loads(raw)
first = open(sys.argv[5]).read()
second = open(sys.argv[6]).read()
ask = sys.argv[7]
if first in raw or second in raw or ask in raw or "body" in raw or "spoken" in raw:
    raise SystemExit("FAIL: final list includes a reply")
notes = data.get("notes")
if set(data.keys()) != {"notes"} or not isinstance(notes, list) or len(notes) != 3:
...
for note in notes:
    ...
    by_id[note["id"]] = note["title"]
want = {
    sys.argv[2].strip(): "Paper note",
    open(sys.argv[3]).read().strip(): ask[:200],
    open(sys.argv[4]).read().strip(): sys.argv[8],
}
if by_id != want:
    raise SystemExit("FAIL: final list notes are %r" % (by_id,))
PY
...
echo "check-second-reply-draft: OK"
