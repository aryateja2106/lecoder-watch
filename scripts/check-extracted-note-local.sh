#!/bin/sh
# One spare daemon holds two extracted pages. Asking the oak page through a
# loopback model sends that sentence and leaves the pairing page out. A query
# that matches both notes is refused before another model call or another draft.
set -eu
...
python3 - "$MODEL" "$STUB_PORT" <<'PY' || exit 1
import sys
...
if parsed.username is not None or parsed.password is not None or "@" in (parsed.netloc or ""):
    raise SystemExit("FAIL: model URL has userinfo")
if parsed.hostname != "127.0.0.1":
    raise SystemExit("FAIL: model host is %r" % (parsed.hostname,))
if parsed.port != int(port) or parsed.path != "/v1":
    raise SystemExit("FAIL: model URL is %r" % (model,))
if "llm.example" in model:
    raise SystemExit("FAIL: model URL names llm.example")
PY
...
python3 - "$CASE/ask-out.json" "$WORK" "$RAN.count" <<'PY' || { echo "FAIL: oak confirmed ask"; cat "$CASE/ask-out.json"; echo; cat "$LOG"; exit 1; }
import json, os, stat, sys
data = json.load(open(sys.argv[1]))
work, count_path = sys.argv[2], sys.argv[3]
...
if b"sk-" in raw or b"pairing code" in raw:
    raise SystemExit("draft contains a key or the other note")
...
echo "check-extracted-note-local: ok"
