...
for name in app-brief apple-native-apis native-app-builder pwa-local-app-builder mesh-knowledge; do
...
# `mesh skills status` must report every shipped skill installed, in JSON a caller can parse.
STATUS_JSON="$(run skills status --json)"
installed_count=$(printf '%s' "$STATUS_JSON" | grep -c '"installed": true') || installed_count=0
[ "$installed_count" -eq 5 ] || bad "mesh skills status --json reports $installed_count/5 installed"
