...
# daemon, then a later query that asks again from only that saved reply.
# The model host is llm.example, mapped to loopback for this check only.
# The class is user-subscription, the stub is called, one relative mode-600
# file is written and not executed, and one knowledge note stores the ask
# as its title and the model reply as its body. A later POST /agent-note
# with no id, a q that matches only that reply, and a new allowed local
# ask sends the saved reply, a blank line, and the ask. It returns
# user-subscription, calls the stub, writes one relative mode-600 file,
# does not execute it, and saves the new reply as another note. That draft
# is not the original source note and not the first reply note. A
# pairing-code second ask writes nothing, does not call the stub, and adds
# no note. A second ask containing :// is 400 and writes nothing. A q that
# matches the original note and the saved reply is 409 and writes nothing.
# Daemon state and held drafts do not keep the host or the key. The
# knowledge list stays {id, title}. Process environ is not read. This
# script does not open /proc. A missing /proc file does not abort it. On
# exit the daemon process group and the stub are killed so the spare port
# is free.
set -eu
...
STUB_PID=""
HOSTS_ADDED=0
HOSTS_MARK="# mesh-subscription-reply-ask"
...
TMP=$(mktemp -d /tmp/mesh-subscription-reply-ask.XXXXXX)
...
printf '%s' 'Noted.' > "$TMP/meta/reply.txt"
: > "$TMP/meta/stub.jsonl"
...
echo "2. query matching only the saved reply sends that reply, a blank line, and the ask"
printf '%s' "$NEXT_REPLY" > "$TMP/meta/reply.txt"
...
python3 - "$TMP/meta/case2.json" "$TMP/meta/before2.txt" "$TMP/meta/after2.txt" "$TMP/work" "$hits_before" "$hits_after" "$TMP/meta/stub.jsonl" "$MESHD_STATE" "$TMP/meta/reply.id" "$TMP/meta/source.id" "$TMP/meta/source.body" "$ASK" "$REPLY_ONLY" "$NEXT_ASK" "$NEXT_REPLY" << 'PY'
import json, os, sys
from urllib.parse import urlsplit
...
if text != next_reply:
    raise SystemExit("drafted file is not the new model reply")
...
if text == first.get("body") or text == first.get("title") or path == os.path.join(work, "held.txt") and text == reply:
    raise SystemExit("drafted file is the first reply note")
if int(hits_after) != int(hits_before) + 1:
...
if note.get("title") != next_ask:
    raise SystemExit("new note title is %r" % note.get("title"))
if note.get("body") != next_reply:
    raise SystemExit("new note body is not the new model reply only")
...
echo "3. pairing-code second ask writes nothing"
...
python3 - "$TMP/meta/case3.json" "$TMP/meta/work-before-pair.txt" "$TMP/meta/work-after-pair.txt" "$TMP/meta/state-before-pair.txt" "$TMP/meta/state-after-pair.txt" "$TMP/work/pairing.txt" "$hits_before" "$hits_after" << 'PY'
import json, os, sys
...
if obj.get("modelClass") != "user-subscription" or obj.get("draft") is not None or obj.get("commandRan") is not False or obj.get("held") is not True:
    raise SystemExit("case 3 was not held: %r" % obj)
...
if lines(state_before) != lines(state_after):
    raise SystemExit("case 3 added a note")
if os.path.exists(pairing):
    raise SystemExit("case 3 wrote pairing.txt")
if int(hits_after) != int(hits_before):
    raise SystemExit("case 3 called the stub")
print("   wrote nothing")
PY

echo "4. second ask containing :// is 400"
file_list "$TMP/work" > "$TMP/meta/work-before-url.txt"
file_list "$MESHD_STATE" > "$TMP/meta/state-before-url.txt"
hits_before=$(hit_count)
python3 - "$TMP/meta/case4.req" "$TMP/work" "$MODEL_URL" "$STUB_PORT" << 'PY'
import json, sys
body = {
    "q": "Noted",
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "remote.txt",
    "ask": "see http://llm.example:%s/reader" % sys.argv[4],
}
if "id" in body:
    raise SystemExit("remote ask included an id")
if "://" not in body["ask"]:
    raise SystemExit("remote ask does not contain ://")
json.dump(body, open(sys.argv[1], "w"))
PY
...
file_list "$TMP/work" > "$TMP/meta/work-after-url.txt"
file_list "$MESHD_STATE" > "$TMP/meta/state-after-url.txt"
python3 - "$TMP/meta/case4.json" "$TMP/meta/work-before-url.txt" "$TMP/meta/work-after-url.txt" "$TMP/meta/state-before-url.txt" "$TMP/meta/state-after-url.txt" "$TMP/work/remote.txt" "$hits_before" "$hits_after" << 'PY'
import json, os, sys
body_path, work_before, work_after, state_before, state_after, remote, hits_before, hits_after = sys.argv[1:]
obj = json.loads(open(body_path, encoding="utf-8").read())
if obj.get("error") != "ask must be local text":
...
if os.path.exists(remote):
    raise SystemExit("case 4 wrote remote.txt")
if int(hits_after) != int(hits_before):
    raise SystemExit("case 4 called the stub")
print("   wrote nothing")
PY

echo "5. query matching the original note and the saved reply is 409"
...
python3 - "$TMP/meta/case5.json" "$TMP/meta/work-before.txt" "$TMP/meta/work-after.txt" "$TMP/meta/state-before.txt" "$TMP/meta/state-after.txt" "$TMP/work/both.txt" "$hits_before" "$hits_after" << 'PY'
import json, os, sys
...
if obj.get("error") != "more than one note":
    raise SystemExit("case 5 body %r" % obj)
if "draft" in obj:
    raise SystemExit("case 5 wrote a draft field")
...
if lines(state_before) != lines(state_after):
    raise SystemExit("case 5 wrote a state file")
if os.path.exists(both):
    raise SystemExit("case 5 wrote both.txt")
if int(hits_after) != int(hits_before):
...
kcode=$(curl -sS --max-time 10 -o "$TMP/meta/knowledge.json" -w '%{http_code}' \
  ...
  "http://127.0.0.1:${MESHD_PORT}/knowledge")
echo "   http ${kcode}"
python3 - "$TMP/meta/knowledge.json" "$ASK" "$NEXT_ASK" "$REPLY_ONLY" "$NEXT_REPLY" << 'PY'
import json, sys
raw = open(sys.argv[1], encoding="utf-8").read()
ask, next_ask, reply, next_reply = sys.argv[2:]
obj = json.loads(raw)
...
if "body" in raw or reply in raw or next_reply in raw or "sk-secret" in raw or "llm.example" in raw or "Local page" in raw:
