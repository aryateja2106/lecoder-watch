...
# spare port is free.
# A later ask of only the confirmed note, titled margin-token-only, sends
# that note and the new ask to the stub. Confirm omitted does not create
# held-ask-ran. Confirm true does. The draft does not keep the host or the key.
# A later query of only the note titled margin-ask-only drafts that saved
# ask. Confirm omitted does not create held-saved-ran. Confirm true does.
# The draft does not keep the host or the key.
# A further ask of only that saved note sends the note and the new ask.
# Confirm omitted does not create held-nested-ran. Confirm true does.
# A user-subscription reply that names a pairing code is held even when
# confirm is true. It writes no file, saves no note, and does not run.
set -eu
...
echo "8. later ask of the confirmed note does not run a command"
python3 - "$TMP/meta/case8.req" "$TMP/work" "$MODEL_URL" << 'PY'
import json, sys
body = {
    "q": "margin-token-only",
    "cwd": sys.argv[2],
    ...
    "file": "held-ask.txt",
    "ask": "margin-ask-only",
    "command": "touch " + sys.argv[2] + "/held-ask-ran",
}
json.dump(body, open(sys.argv[1], "w"))
PY
before=$(mode600_list || true)
hits_before=$(hit_count)
code=$(post_note "$TMP/meta/case8.req" "$TMP/meta/case8.json")
echo "   http ${code}"
hits_after=$(hit_count)
if [ "$code" != "200" ]; then
  cat "$TMP/meta/case8.json" >&2 || true
  exit 1
fi
if [ -e "$TMP/work/held-ask-ran" ]; then
  echo "later ask ran the command without confirm" >&2
  exit 1
fi
printf '%s\n' "$before" > "$TMP/meta/before8.txt"
mode600_list > "$TMP/meta/after8.txt" || true
python3 - "$TMP/meta/case8.json" "$TMP/meta/before8.txt" "$TMP/meta/after8.txt" "$TMP/work" "$hits_before" "$hits_after" "$TMP/meta/stub.jsonl" "$REPLY" "$TMP/meta/case8.draft" << 'PY'
import json, os, sys
from urllib.parse import urlsplit
body_path, before_path, after_path, work, hits_before, hits_after, stub_path, reply, draft_out = sys.argv[1:]
raw = open(body_path, encoding="utf-8").read()
...
draft = obj["draft"]
if not isinstance(draft, str) or not draft or os.path.isabs(draft) or draft.startswith("..") or "/" in draft:
    raise SystemExit("case 8 draft is not one relative file: %r" % draft)
before = {ln for ln in open(before_path, encoding="utf-8").read().splitlines() if ln}
after = {ln for ln in open(after_path, encoding="utf-8").read().splitlines() if ln}
new = sorted(after - before)
if new != [os.path.join(work, draft)]:
    raise SystemExit("case 8 expected one new mode-600 file, got %s" % new)
path = new[0]
mode = os.stat(path).st_mode & 0o777
if mode != 0o600 or mode & 0o111:
    raise SystemExit("case 8 mode %o" % mode)
if open(path, encoding="utf-8").read() != reply:
    raise SystemExit("case 8 draft is not the stub reply")
if int(hits_after) != int(hits_before) + 1:
    raise SystemExit("case 8 stub hits %s -> %s" % (hits_before, hits_after))
recs = [json.loads(ln) for ln in open(stub_path, encoding="utf-8").read().splitlines() if ln.strip()]
rec = recs[int(hits_before)]
url = rec.get("url") or ""
parts = urlsplit(url)
if parts.username or parts.password or "@" in url:
    raise SystemExit("case 8 stub url has userinfo")
sent = rec.get("body") or ""
if "The margin stays beside the sentence." not in sent or "margin-ask-only" not in sent or "margin-token-only" not in sent:
    raise SystemExit("case 8 did not send the confirmed note and the ask")
if "sk-secret" in sent or "llm.example" in sent:
    raise SystemExit("case 8 stub body contains the host or the key")
open(draft_out, "w", encoding="utf-8").write(path)
print("   file", draft)
PY
DRAFT8=$(cat "$TMP/meta/case8.draft")
scan_absent "$MESHD_STATE" "$DRAFT8" "needle:sk-secret" "needle:llm.example"
...
python3 - "$TMP/meta/knowledge-ask.json" << 'PY'
import json, sys
raw = open(sys.argv[1], encoding="utf-8").read()
if "sk-secret" in raw or "llm.example" in raw or "The margin stays beside the sentence." in raw:
    raise SystemExit("case 8 knowledge list leaked a body or a model url")
obj = json.loads(raw)
titles = [item["title"] for item in obj["notes"]]
if titles.count("margin-ask-only") != 1:
    raise SystemExit("case 8 ask title count %s" % titles)
print("   ask note saved")
PY

echo "9. the same later ask with confirm true runs the command"
python3 - "$TMP/meta/case9.req" "$TMP/work" "$MODEL_URL" << 'PY'
import json, sys
body = {
    "q": "margin-token-only",
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "held-ask-true.txt",
    "ask": "margin-ask-true",
    "command": "touch " + sys.argv[2] + "/held-ask-ran",
    "confirm": True,
}
...
before=$(mode600_list || true)
hits_before=$(hit_count)
code=$(post_note "$TMP/meta/case9.req" "$TMP/meta/case9.json")
echo "   http ${code}"
hits_after=$(hit_count)
if [ "$code" != "200" ]; then
  cat "$TMP/meta/case9.json" >&2 || true
  exit 1
fi
if [ ! -e "$TMP/work/held-ask-ran" ]; then
  echo "later ask did not run the command when confirm is true" >&2
  exit 1
fi
printf '%s\n' "$before" > "$TMP/meta/before9.txt"
mode600_list > "$TMP/meta/after9.txt" || true
python3 - "$TMP/meta/case9.json" "$TMP/meta/before9.txt" "$TMP/meta/after9.txt" "$TMP/work" "$hits_before" "$hits_after" "$TMP/meta/stub.jsonl" "$REPLY" "$TMP/meta/case9.draft" << 'PY'
import json, os, sys
from urllib.parse import urlsplit
body_path, before_path, after_path, work, hits_before, hits_after, stub_path, reply, draft_out = sys.argv[1:]
raw = open(body_path, encoding="utf-8").read()
...
draft = obj["draft"]
if not isinstance(draft, str) or not draft or os.path.isabs(draft) or "/" in draft:
    raise SystemExit("case 9 draft is not one relative file: %r" % draft)
before = {ln for ln in open(before_path, encoding="utf-8").read().splitlines() if ln}
after = {ln for ln in open(after_path, encoding="utf-8").read().splitlines() if ln}
new = sorted(after - before)
if new != [os.path.join(work, draft)]:
    raise SystemExit("case 9 expected one new mode-600 file, got %s" % new)
path = new[0]
mode = os.stat(path).st_mode & 0o777
if mode != 0o600 or mode & 0o111:
    raise SystemExit("case 9 mode %o" % mode)
if open(path, encoding="utf-8").read() != reply:
    raise SystemExit("case 9 draft is not the stub reply")
if int(hits_after) != int(hits_before) + 1:
    raise SystemExit("case 9 stub hits %s -> %s" % (hits_before, hits_after))
recs = [json.loads(ln) for ln in open(stub_path, encoding="utf-8").read().splitlines() if ln.strip()]
rec = recs[int(hits_before)]
url = rec.get("url") or ""
parts = urlsplit(url)
if parts.username or parts.password or "@" in url:
    raise SystemExit("case 9 stub url has userinfo")
sent = rec.get("body") or ""
if "The margin stays beside the sentence." not in sent or "margin-ask-true" not in sent:
    raise SystemExit("case 9 did not send the confirmed note and the ask")
if "sk-secret" in sent or "llm.example" in sent:
    raise SystemExit("case 9 stub body contains the host or the key")
open(draft_out, "w", encoding="utf-8").write(path)
print("   file", draft)
PY
DRAFT9=$(cat "$TMP/meta/case9.draft")
scan_absent "$MESHD_STATE" "$DRAFT8" "$DRAFT9" "needle:sk-secret" "needle:llm.example"

echo "10. later query of the saved ask does not run a command"
python3 - "$TMP/meta/case10.req" "$TMP/work" "$MODEL_URL" << 'PY'
import json, sys
body = {
    "q": "margin-ask-only",
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "held-saved.txt",
    "command": "touch " + sys.argv[2] + "/held-saved-ran",
}
json.dump(body, open(sys.argv[1], "w"))
PY
before=$(mode600_list || true)
hits_before=$(hit_count)
code=$(post_note "$TMP/meta/case10.req" "$TMP/meta/case10.json")
echo "   http ${code}"
hits_after=$(hit_count)
if [ "$code" != "200" ]; then
  cat "$TMP/meta/case10.json" >&2 || true
  exit 1
fi
if [ -e "$TMP/work/held-saved-ran" ]; then
  echo "saved-ask query ran the command without confirm" >&2
  exit 1
fi
printf '%s\n' "$before" > "$TMP/meta/before10.txt"
mode600_list > "$TMP/meta/after10.txt" || true
python3 - "$TMP/meta/case10.json" "$TMP/meta/before10.txt" "$TMP/meta/after10.txt" "$TMP/work" "$hits_before" "$hits_after" "$TMP/meta/stub.jsonl" "$REPLY" "$TMP/meta/case10.draft" << 'PY'
import json, os, sys
from urllib.parse import urlsplit
body_path, before_path, after_path, work, hits_before, hits_after, stub_path, reply, draft_out = sys.argv[1:]
raw = open(body_path, encoding="utf-8").read()
...
draft = obj["draft"]
if not isinstance(draft, str) or not draft or os.path.isabs(draft) or draft.startswith("..") or "/" in draft:
    raise SystemExit("case 10 draft is not one relative file: %r" % draft)
before = {ln for ln in open(before_path, encoding="utf-8").read().splitlines() if ln}
after = {ln for ln in open(after_path, encoding="utf-8").read().splitlines() if ln}
new = sorted(after - before)
if new != [os.path.join(work, draft)]:
    raise SystemExit("case 10 expected one new mode-600 file, got %s" % new)
path = new[0]
mode = os.stat(path).st_mode & 0o777
if mode != 0o600 or mode & 0o111:
    raise SystemExit("case 10 mode %o" % mode)
if open(path, encoding="utf-8").read() != reply:
    raise SystemExit("case 10 draft is not the stub reply")
if int(hits_after) != int(hits_before) + 1:
    raise SystemExit("case 10 stub hits %s -> %s" % (hits_before, hits_after))
recs = [json.loads(ln) for ln in open(stub_path, encoding="utf-8").read().splitlines() if ln.strip()]
rec = recs[int(hits_before)]
url = rec.get("url") or ""
parts = urlsplit(url)
if parts.username or parts.password or "@" in url:
    raise SystemExit("case 10 stub url has userinfo")
sent = rec.get("body") or ""
if "The margin stays beside the sentence." not in sent or "margin-ask-only" not in sent:
    raise SystemExit("case 10 did not send the saved ask")
if "sk-secret" in sent or "llm.example" in sent:
    raise SystemExit("case 10 stub body contains the host or the key")
open(draft_out, "w", encoding="utf-8").write(path)
print("   file", draft)
PY
DRAFT10=$(cat "$TMP/meta/case10.draft")
scan_absent "$MESHD_STATE" "$DRAFT10" "needle:sk-secret" "needle:llm.example"

echo "11. the same saved-ask query with confirm true runs the command"
python3 - "$TMP/meta/case11.req" "$TMP/work" "$MODEL_URL" << 'PY'
import json, sys
body = {
    "q": "margin-ask-only",
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "held-saved-true.txt",
    "command": "touch " + sys.argv[2] + "/held-saved-ran",
    "confirm": True,
}
json.dump(body, open(sys.argv[1], "w"))
...
before=$(mode600_list || true)
hits_before=$(hit_count)
code=$(post_note "$TMP/meta/case11.req" "$TMP/meta/case11.json")
echo "   http ${code}"
hits_after=$(hit_count)
if [ "$code" != "200" ]; then
  cat "$TMP/meta/case11.json" >&2 || true
  exit 1
fi
if [ ! -e "$TMP/work/held-saved-ran" ]; then
  echo "saved-ask query did not run the command when confirm is true" >&2
  exit 1
fi
printf '%s\n' "$before" > "$TMP/meta/before11.txt"
mode600_list > "$TMP/meta/after11.txt" || true
python3 - "$TMP/meta/case11.json" "$TMP/meta/before11.txt" "$TMP/meta/after11.txt" "$TMP/work" "$hits_before" "$hits_after" "$TMP/meta/stub.jsonl" "$REPLY" "$TMP/meta/case11.draft" << 'PY'
import json, os, sys
from urllib.parse import urlsplit
body_path, before_path, after_path, work, hits_before, hits_after, stub_path, reply, draft_out = sys.argv[1:]
raw = open(body_path, encoding="utf-8").read()
...
draft = obj["draft"]
if not isinstance(draft, str) or not draft or os.path.isabs(draft) or "/" in draft:
    raise SystemExit("case 11 draft is not one relative file: %r" % draft)
before = {ln for ln in open(before_path, encoding="utf-8").read().splitlines() if ln}
after = {ln for ln in open(after_path, encoding="utf-8").read().splitlines() if ln}
new = sorted(after - before)
if new != [os.path.join(work, draft)]:
    raise SystemExit("case 11 expected one new mode-600 file, got %s" % new)
path = new[0]
mode = os.stat(path).st_mode & 0o777
if mode != 0o600 or mode & 0o111:
    raise SystemExit("case 11 mode %o" % mode)
if open(path, encoding="utf-8").read() != reply:
    raise SystemExit("case 11 draft is not the stub reply")
if int(hits_after) != int(hits_before) + 1:
    raise SystemExit("case 11 stub hits %s -> %s" % (hits_before, hits_after))
recs = [json.loads(ln) for ln in open(stub_path, encoding="utf-8").read().splitlines() if ln.strip()]
rec = recs[int(hits_before)]
url = rec.get("url") or ""
parts = urlsplit(url)
if parts.username or parts.password or "@" in url:
    raise SystemExit("case 11 stub url has userinfo")
sent = rec.get("body") or ""
if "The margin stays beside the sentence." not in sent or "margin-ask-only" not in sent:
    raise SystemExit("case 11 did not send the saved ask")
if "sk-secret" in sent or "llm.example" in sent:
    raise SystemExit("case 11 stub body contains the host or the key")
open(draft_out, "w", encoding="utf-8").write(path)
print("   file", draft)
PY
DRAFT11=$(cat "$TMP/meta/case11.draft")
scan_absent "$MESHD_STATE" "$DRAFT10" "$DRAFT11" "needle:sk-secret" "needle:llm.example"

echo "12. further ask of the saved note does not run a command"
python3 - "$TMP/meta/case12.req" "$TMP/work" "$MODEL_URL" << 'PY'
import json, sys
body = {
    "q": "margin-ask-only",
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "held-nested.txt",
    "ask": "margin-saved-ask",
    "command": "touch " + sys.argv[2] + "/held-nested-ran",
}
json.dump(body, open(sys.argv[1], "w"))
PY
before=$(mode600_list || true)
hits_before=$(hit_count)
code=$(post_note "$TMP/meta/case12.req" "$TMP/meta/case12.json")
echo "   http ${code}"
hits_after=$(hit_count)
if [ "$code" != "200" ]; then
  cat "$TMP/meta/case12.json" >&2 || true
  exit 1
fi
if [ -e "$TMP/work/held-nested-ran" ]; then
  echo "further ask ran the command without confirm" >&2
  exit 1
fi
printf '%s\n' "$before" > "$TMP/meta/before12.txt"
mode600_list > "$TMP/meta/after12.txt" || true
python3 - "$TMP/meta/case12.json" "$TMP/meta/before12.txt" "$TMP/meta/after12.txt" "$TMP/work" "$hits_before" "$hits_after" "$TMP/meta/stub.jsonl" "$REPLY" "$TMP/meta/case12.draft" << 'PY'
import json, os, sys
from urllib.parse import urlsplit
body_path, before_path, after_path, work, hits_before, hits_after, stub_path, reply, draft_out = sys.argv[1:]
raw = open(body_path, encoding="utf-8").read()
if "sk-secret" in raw or "llm.example" in raw:
    raise SystemExit("case 12 response contains the host or the key")
obj = json.loads(raw)
if set(obj.keys()) != {"modelClass", "draft", "commandRan", "held", "reply"}:
    raise SystemExit("case 12 keys %s" % sorted(obj.keys()))
if obj["modelClass"] != "user-subscription" or obj["commandRan"] is not False or obj["held"] is not True:
    raise SystemExit("case 12 ran or was not held: %r" % obj)
draft = obj["draft"]
if not isinstance(draft, str) or not draft or os.path.isabs(draft) or draft.startswith("..") or "/" in draft:
    raise SystemExit("case 12 draft is not one relative file: %r" % draft)
before = {ln for ln in open(before_path, encoding="utf-8").read().splitlines() if ln}
after = {ln for ln in open(after_path, encoding="utf-8").read().splitlines() if ln}
new = sorted(after - before)
if new != [os.path.join(work, draft)]:
    raise SystemExit("case 12 expected one new mode-600 file, got %s" % new)
path = new[0]
mode = os.stat(path).st_mode & 0o777
if mode != 0o600 or mode & 0o111:
    raise SystemExit("case 12 mode %o" % mode)
if open(path, encoding="utf-8").read() != reply:
    raise SystemExit("case 12 draft is not the stub reply")
if int(hits_after) != int(hits_before) + 1:
    raise SystemExit("case 12 stub hits %s -> %s" % (hits_before, hits_after))
recs = [json.loads(ln) for ln in open(stub_path, encoding="utf-8").read().splitlines() if ln.strip()]
rec = recs[int(hits_before)]
url = rec.get("url") or ""
parts = urlsplit(url)
if parts.username or parts.password or "@" in url:
    raise SystemExit("case 12 stub url has userinfo")
sent = rec.get("body") or ""
for needle in ("The margin stays beside the sentence.", "margin-ask-only", "margin-saved-ask"):
    if needle not in sent:
        raise SystemExit("case 12 did not send %s" % needle)
if "sk-secret" in sent or "llm.example" in sent:
    raise SystemExit("case 12 stub body contains the host or the key")
open(draft_out, "w", encoding="utf-8").write(path)
print("   file", draft)
PY
DRAFT12=$(cat "$TMP/meta/case12.draft")
scan_absent "$MESHD_STATE" "$DRAFT12" "needle:sk-secret" "needle:llm.example"
kcode=$(curl -sS --max-time 10 -o "$TMP/meta/knowledge-nested.json" -w '%{http_code}' \
  -H "Authorization: Bearer ${MESHD_TOKEN}" \
  "http://127.0.0.1:${MESHD_PORT}/knowledge")
echo "   knowledge ${kcode}"
if [ "$kcode" != "200" ]; then
...
python3 - "$TMP/meta/knowledge-nested.json" << 'PY'
import json, sys
raw = open(sys.argv[1], encoding="utf-8").read()
if "sk-secret" in raw or "llm.example" in raw or "The margin stays beside the sentence." in raw:
    raise SystemExit("case 12 knowledge list leaked a body or a model url")
obj = json.loads(raw)
titles = [item["title"] for item in obj["notes"]]
if titles.count("margin-saved-ask") != 1:
    raise SystemExit("case 12 saved ask title count %s" % titles)
print("   further ask note saved")
PY

echo "13. the same further ask with confirm true runs the command"
python3 - "$TMP/meta/case13.req" "$TMP/work" "$MODEL_URL" << 'PY'
import json, sys
body = {
    "q": "margin-ask-only",
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "held-nested-true.txt",
    "ask": "margin-saved-true",
    "command": "touch " + sys.argv[2] + "/held-nested-ran",
    "confirm": True,
}
json.dump(body, open(sys.argv[1], "w"))
...
before=$(mode600_list || true)
hits_before=$(hit_count)
code=$(post_note "$TMP/meta/case13.req" "$TMP/meta/case13.json")
echo "   http ${code}"
hits_after=$(hit_count)
if [ "$code" != "200" ]; then
  cat "$TMP/meta/case13.json" >&2 || true
  exit 1
fi
if [ ! -e "$TMP/work/held-nested-ran" ]; then
  echo "further ask did not run the command when confirm is true" >&2
  exit 1
fi
printf '%s\n' "$before" > "$TMP/meta/before13.txt"
mode600_list > "$TMP/meta/after13.txt" || true
python3 - "$TMP/meta/case13.json" "$TMP/meta/before13.txt" "$TMP/meta/after13.txt" "$TMP/work" "$hits_before" "$hits_after" "$TMP/meta/stub.jsonl" "$REPLY" "$TMP/meta/case13.draft" << 'PY'
import json, os, sys
from urllib.parse import urlsplit
body_path, before_path, after_path, work, hits_before, hits_after, stub_path, reply, draft_out = sys.argv[1:]
raw = open(body_path, encoding="utf-8").read()
if "sk-secret" in raw or "llm.example" in raw:
    raise SystemExit("case 13 response contains the host or the key")
obj = json.loads(raw)
if set(obj.keys()) != {"modelClass", "draft", "commandRan", "held", "reply"}:
    raise SystemExit("case 13 keys %s" % sorted(obj.keys()))
if obj["modelClass"] != "user-subscription" or obj["commandRan"] is not True or obj["held"] is not False:
    raise SystemExit("case 13 did not run: %r" % obj)
draft = obj["draft"]
if not isinstance(draft, str) or not draft or os.path.isabs(draft) or "/" in draft:
    raise SystemExit("case 13 draft is not one relative file: %r" % draft)
before = {ln for ln in open(before_path, encoding="utf-8").read().splitlines() if ln}
after = {ln for ln in open(after_path, encoding="utf-8").read().splitlines() if ln}
new = sorted(after - before)
if new != [os.path.join(work, draft)]:
    raise SystemExit("case 13 expected one new mode-600 file, got %s" % new)
path = new[0]
mode = os.stat(path).st_mode & 0o777
if mode != 0o600 or mode & 0o111:
    raise SystemExit("case 13 mode %o" % mode)
if open(path, encoding="utf-8").read() != reply:
    raise SystemExit("case 13 draft is not the stub reply")
if int(hits_after) != int(hits_before) + 1:
    raise SystemExit("case 13 stub hits %s -> %s" % (hits_before, hits_after))
recs = [json.loads(ln) for ln in open(stub_path, encoding="utf-8").read().splitlines() if ln.strip()]
rec = recs[int(hits_before)]
url = rec.get("url") or ""
parts = urlsplit(url)
if parts.username or parts.password or "@" in url:
    raise SystemExit("case 13 stub url has userinfo")
sent = rec.get("body") or ""
for needle in ("The margin stays beside the sentence.", "margin-ask-only", "margin-saved-true"):
    if needle not in sent:
        raise SystemExit("case 13 did not send %s" % needle)
if "sk-secret" in sent or "llm.example" in sent:
    raise SystemExit("case 13 stub body contains the host or the key")
open(draft_out, "w", encoding="utf-8").write(path)
print("   file", draft)
PY
DRAFT13=$(cat "$TMP/meta/case13.draft")
scan_absent "$MESHD_STATE" "$DRAFT12" "$DRAFT13" "needle:sk-secret" "needle:llm.example"

echo "14. a secret subscription reply stays held when confirm is true"
cp "$TMP/meta/reply.txt" "$TMP/meta/reply-safe.txt"
printf '%s' 'The pairing code stays on this machine.' > "$TMP/meta/reply.txt"
python3 - "$TMP/meta/case14.req" "$TMP/work" "$MODEL_URL" << 'PY'
import json, sys
body = {
    "q": "margin-ask-only",
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "held-secret.txt",
    "ask": "margin-secret-reply",
    "command": "touch " + sys.argv[2] + "/held-secret-ran",
    "confirm": True,
}
json.dump(body, open(sys.argv[1], "w"))
PY
before=$(mode600_list || true)
hits_before=$(hit_count)
code=$(post_note "$TMP/meta/case14.req" "$TMP/meta/case14.json")
cp "$TMP/meta/reply-safe.txt" "$TMP/meta/reply.txt"
echo "   http ${code}"
hits_after=$(hit_count)
if [ "$code" != "200" ]; then
  cat "$TMP/meta/case14.json" >&2 || true
  exit 1
fi
if [ -e "$TMP/work/held-secret-ran" ] || [ -e "$TMP/work/held-secret.txt" ]; then
  echo "secret reply wrote a file or ran the command" >&2
  exit 1
fi
printf '%s\n' "$before" > "$TMP/meta/before14.txt"
mode600_list > "$TMP/meta/after14.txt" || true
python3 - "$TMP/meta/case14.json" "$TMP/meta/before14.txt" "$TMP/meta/after14.txt" "$hits_before" "$hits_after" "$TMP/meta/stub.jsonl" << 'PY'
import json, sys
from urllib.parse import urlsplit
body_path, before_path, after_path, hits_before, hits_after, stub_path = sys.argv[1:]
raw = open(body_path, encoding="utf-8").read()
if "sk-secret" in raw or "llm.example" in raw or "pairing code" in raw:
    raise SystemExit("case 14 response contains a secret or the host")
obj = json.loads(raw)
if set(obj.keys()) != {"modelClass", "draft", "commandRan", "held", "reply"}:
    raise SystemExit("case 14 keys %s" % sorted(obj.keys()))
if obj["modelClass"] != "user-subscription" or obj["draft"] is not None or obj["commandRan"] is not False or obj["held"] is not True:
    raise SystemExit("case 14 was not held: %r" % obj)
before = [ln for ln in open(before_path, encoding="utf-8").read().splitlines() if ln]
after = [ln for ln in open(after_path, encoding="utf-8").read().splitlines() if ln]
if before != after:
    raise SystemExit("case 14 wrote a mode-600 file")
if int(hits_after) != int(hits_before) + 1:
    raise SystemExit("case 14 stub hits %s -> %s" % (hits_before, hits_after))
recs = [json.loads(ln) for ln in open(stub_path, encoding="utf-8").read().splitlines() if ln.strip()]
rec = recs[int(hits_before)]
url = rec.get("url") or ""
parts = urlsplit(url)
if parts.username or parts.password or "@" in url:
    raise SystemExit("case 14 stub url has userinfo")
sent = rec.get("body") or ""
if "margin-secret-reply" not in sent or "margin-ask-only" not in sent:
    raise SystemExit("case 14 did not send the saved note and the ask")
if "sk-secret" in sent or "llm.example" in sent:
    raise SystemExit("case 14 stub body contains the host or the key")
print("   held")
PY
kcode=$(curl -sS --max-time 10 -o "$TMP/meta/knowledge-secret.json" -w '%{http_code}' \
  -H "Authorization: Bearer ${MESHD_TOKEN}" \
  "http://127.0.0.1:${MESHD_PORT}/knowledge")
echo "   knowledge ${kcode}"
if [ "$kcode" != "200" ]; then
...
python3 - "$TMP/meta/knowledge-secret.json" << 'PY'
import json, sys
raw = open(sys.argv[1], encoding="utf-8").read()
if "sk-secret" in raw or "llm.example" in raw or "pairing code" in raw:
    raise SystemExit("case 14 knowledge list leaked a secret or a model url")
titles = [item["title"] for item in json.loads(raw)["notes"]]
if "margin-secret-reply" in titles:
    raise SystemExit("case 14 saved the secret reply")
print("   no secret note")
PY
scan_absent "$MESHD_STATE" "needle:sk-secret" "needle:llm.example"
