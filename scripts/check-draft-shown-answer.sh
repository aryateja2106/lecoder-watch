#!/bin/sh
# Draft one saved note by id. The body is only id and confirm.
# A local model writes draft.txt and does not run it. An empty or remote
# model is refused. A note whose body names a pairing code is held.
set -eu
...
TH="$(mktemp -d /tmp/draft-shown-answer.XXXXXX)"
PORT=8898
SRV=
STUB=
...
# Model stub on loopback. The draft request never sends this URL.
...
# Local MESH_MODEL: id and confirm write the stub sentence and leave the note alone.
C1="$TH/local"
...
echo "check-draft-shown-answer: local model wrote draft.txt"
stop_srv
wait_port_free

# Empty MESH_MODEL: refuse before a draft.
C2="$TH/empty"
...
echo "check-draft-shown-answer: empty model refused"
stop_srv
wait_port_free
...
echo "check-draft-shown-answer: non-local model refused"
stop_srv
wait_port_free

# A note whose body names a pairing code is held. No draft, and the stub stays quiet.
C4="$TH/pair"
...
echo "check-draft-shown-answer: pairing note held"
stop_srv
wait_port_free

echo "check-draft-shown-answer: ok"
