#!/bin/sh
# Spare daemon on 127.0.0.1:8898. An allowed ask saves one reply note.
# A later POST /agent-note with no id, a q that matches only that reply,
# and a new local ask sends the saved reply, a blank line, and the ask.
# It writes one relative file, mode 600, and does not execute it, then
# saves the new reply as another note. A pairing-code second ask does not
# call the model, writes no draft, and adds no note. A second ask
# containing :// is 400 and writes nothing. A q that matches the original
# note and the saved reply is 409 and writes nothing, even with an ask.
...
TH="$(mktemp -d /tmp/saved-reply-ask.XXXXXX)"
HOME_DIR="$TH/home"
STATE="$TH/state"
WORK="$TH/session"
LOG="$TH/meshd.log"
...
NEXT='The margin stays beside the sentence.'
FIRST_ASK='build a reader'
NEXT_ASK='add a margin'
...
echo "check-saved-reply-ask: paper note is stored"
...
if by_id.get(paper) != "Paper note" or by_id.get(reply) != ask:
    raise SystemExit("FAIL: list notes are %r" % (by_id,))
PY
echo "check-saved-reply-ask: allowed ask saved one reply note"
...
echo "check-saved-reply-ask: query matching the original note and the reply returned 409"
...
before_notes="$(note_count)"
files_before="$(work_files)"
ONLY_REPLY="$ONLY_REPLY" NEXT_ASK="$NEXT_ASK" python3 - "$TH/loop.json" "$WORK" "$MODEL" <<'PY'
import json, os, sys
body = {
...
NEXT_ASK="$NEXT_ASK" python3 - "$TH/loop.out" "$WORK/follow.txt" "$STATE/knowledge/$(cat "$TH/reply.id").json" "$WORK/executed-follow" "$TH/stub-bodies/2.json" "$TH/next.txt" "$STATE" "$PAPER_ID" "$TH/reply.id" "$TH/next.id" <<'PY'
import json, os, stat, sys
...
PY
echo "check-saved-reply-ask: query matching only the saved reply sent the reply, a blank line, and the ask"

before_hits="$(stub_hits)"
before_notes="$(note_count)"
files_before="$(work_files)"
ONLY_REPLY="$ONLY_REPLY" python3 - "$TH/pair.json" "$WORK" "$MODEL" <<'PY'
import json, os, sys
body = {
    "q": os.environ["ONLY_REPLY"],
    "cwd": sys.argv[2],
...
echo "check-saved-reply-ask: pairing-code second ask wrote nothing"

before_hits="$(stub_hits)"
before_notes="$(note_count)"
files_before="$(work_files)"
ONLY_REPLY="$ONLY_REPLY" python3 - "$TH/remote.json" "$WORK" "$MODEL" "$STUB_PORT" <<'PY'
import json, os, sys
body = {
    "q": os.environ["ONLY_REPLY"],
    "cwd": sys.argv[2],
    "model": sys.argv[3],
    "file": "remote.txt",
    "command": "touch executed-remote",
    "ask": "http://127.0.0.1:%s/reader" % sys.argv[4],
}
if "id" in body:
    raise SystemExit("FAIL: remote ask included an id")
if "://" not in body["ask"]:
    raise SystemExit("FAIL: remote ask does not contain ://")
json.dump(body, open(sys.argv[1], "w"))
PY
code="$(post_json "$TH/remote.out" "$TH/remote.json" "/agent-note")"
[ "$code" = "400" ] || { echo "FAIL: remote ask -> ${code}"; cat "$TH/remote.out"; echo; exit 1; }
python3 - "$TH/remote.out" <<'PY'
import json, sys
data = json.load(open(sys.argv[1]))
if data != {"error": "ask must be local text"}:
    raise SystemExit("FAIL: remote ask body is %r" % (data,))
PY
[ "$(stub_hits)" = "$before_hits" ] || { echo "FAIL: remote ask called the model"; exit 1; }
[ "$(note_count)" = "$before_notes" ] || { echo "FAIL: remote ask added a note"; exit 1; }
[ "$(work_files)" = "$files_before" ] || { echo "FAIL: remote ask wrote a file"; exit 1; }
[ ! -e "$WORK/remote.txt" ] || { echo "FAIL: remote ask wrote a draft"; exit 1; }
[ ! -e "$WORK/executed-remote" ] || { echo "FAIL: remote ask executed a command"; exit 1; }
echo "check-saved-reply-ask: second ask containing :// stayed 400"
...
python3 - "$TH/list2.json" "$PAPER_ID" "$TH/reply.id" "$TH/next.id" "$TH/assistant.txt" "$TH/next.txt" "$FIRST_ASK" "$NEXT_ASK" <<'PY'
import json, sys
raw = open(sys.argv[1]).read()
data = json.loads(raw)
sentence = open(sys.argv[5]).read()
fresh = open(sys.argv[6]).read()
if sentence in raw or fresh in raw or "body" in raw or "spoken" in raw:
...
want = {
    ...
    open(sys.argv[3]).read().strip(): sys.argv[7],
