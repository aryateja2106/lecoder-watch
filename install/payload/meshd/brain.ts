...
//
// Several engines are supported on purpose: a local server the user already runs (LM
// Studio, ollama) and the streaming ones we measure (edge0 on Apple Silicon, mference). All
// speak OpenAI Chat Completions, so this module treats them identically and never starts,
// stops or reconfigures any of them — meshd fronts them, it does not own them.
//
...
// Order = preference when several answer. edge0 (MLX, Apple Silicon) and ollama (the Linux
// boxes: Jetson CUDA, Pi CPU) joined 2026-09-22; mference and LM Studio stay for the Macs
// that run them. All four speak OpenAI Chat Completions.
const DEFAULT_CANDIDATES: Array<{ source: string; endpoint: string }> = [
  { source: "edge0", endpoint: "http://127.0.0.1:8001/v1" },
  { source: "mference", endpoint: "http://127.0.0.1:8080/v1" },
  { source: "ollama", endpoint: "http://127.0.0.1:11434/v1" },
