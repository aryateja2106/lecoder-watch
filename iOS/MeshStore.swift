...
final class MeshStore: ObservableObject {
    ...
    func askCurrentKnowledgeNote(host: String, ask: String, confirmed: Bool) {
        ...
        guard !q.isEmpty, !question.isEmpty else {
            fail("Name the note and the question first")
            return
        }
        guard let machine = machineMatching(host, in: machines),
              let snap = snapshot?.machines.first(where: { $0.host == machine.host }),
              snap.reachable, snap.authError == nil else {
            fail("Ask needs a direct link to this Mac")
            return
        }
        lastError = nil
        let c = client(for: machine)
    ...
    /// Draft the saved answer on screen. Its title is the question's first 200 characters.
    /// Nothing is posted until the user confirms. A missing or ambiguous title does not draft.
    func draftShownAnswer(host: String, question: String, confirmed: Bool) {
        guard confirmed else { return }
        guard let shown = knowledgeAnswer, !shown.isEmpty else { return }
        let titled = String(question.trimmingCharacters(in: .whitespacesAndNewlines).prefix(200))
        guard let machine = machineMatching(host, in: machines),
              let snap = snapshot?.machines.first(where: { $0.host == machine.host }),
              snap.reachable, snap.authError == nil else {
            fail("Draft needs a direct link to this Mac")
            return
        }
        lastError = nil
        let c = client(for: machine)
    ...
    /// Save the reply on screen into the note whose title is the question.
    /// Nothing is posted until the user confirms. A missing or ambiguous title does not write.
    func saveShownReply(host: String, question: String, confirmed: Bool) {
        guard confirmed else { return }
        guard let shown = knowledgeAnswer, !shown.isEmpty else { return }
        let titled = String(question.trimmingCharacters(in: .whitespacesAndNewlines).prefix(200))
        guard let machine = machineMatching(host, in: machines),
