...
- **The terminal is a terminal.** *Open terminal* on a session paints the pane with a real emulator (SwiftTerm): colour, cursor, the agent's own layout, wider panes scroll sideways, pinch sets the font. A fixed key bar under it — Ctrl and Alt stick for one key, Esc, Tab, ↑, dictate, keyboard — and the system keyboard types straight into the pane, in order. Needs a 0.8 daemon (`captureAnsi`); older ones keep the bridge page.
