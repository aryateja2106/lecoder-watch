...
- **The terminal is a terminal.** *Open terminal* on a session attaches the pane over a live stream (`/agents/:s/pty`, a real pty on the machine sized to your phone) and paints it with a real emulator (SwiftTerm): colour, cursor, the agent's own layout, tmux's status line, pinch sets the font; it reconnects by itself. A fixed key bar under it — Ctrl and Alt stick for one key, Esc, Tab, ↑, dictate, keyboard — and the system keyboard types straight into the pane, in order. Needs a 0.8 daemon; older ones keep the bridge page.
