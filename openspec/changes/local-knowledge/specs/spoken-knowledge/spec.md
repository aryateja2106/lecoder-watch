...
- **THEN** the agent records that both checks exited 0 and records that Daemon CI is green on `0a3fefb` (check run `35758382137`). Apps (Xcode) was still running when last checked. Do not claim both jobs are green on `0a3fefb`.
