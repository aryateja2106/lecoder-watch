...
- **WHEN** an agent re-checks pull request 139, 145, 154, 160, 162, 174, 176, 182, 183, 187, 189, 193, 195, 197, 198, 199, 200, 201, 202, 203, 204, 205, or 206 and the tip still matches the design
...
No task in this change requires Arya's physical iPhone, Apple Watch, microphone, or speaker. An agent MUST NOT claim that a microphone or a speaker was proven. A spoken note is a local binary transcript or a local TTS exit code. Pull request 200 does not prove a speaker played audio. Pull request 202 does not prove a speaker played audio or that a paper was read. Pull request 203 does not prove a speaker or that a paper was read. Pull request 204 does not prove a speaker or that a paper was read. Pull request 205 does not prove a speaker or that a paper was read. Pull request 206 does not prove a speaker or that a paper was read. Daemon CI is green on `0de3ba9` (check run `35754767900`). Apps (Xcode) is still running on that run. Do not claim both jobs are green on `0de3ba9`. Daemon CI and Xcode CI are both green on handoff tip `168d08e` (check run `35754719132`). Those device checks are handed back to a human.
...
#### Scenario: Daemon-only CI on the second reply draft is not both jobs green

- **WHEN** an agent is about to claim both jobs are green on `0de3ba9`
- **THEN** the agent records that daemon CI is green on `0de3ba9` (check run `35754767900`) while apps (Xcode) is still running
- **AND** the agent does not claim both jobs are green on `0de3ba9`
