...
- **WHEN** an agent re-checks pull request 139, 145, 154, 160, 162, 174, 176, 182, 183, 187, 189, 193, 195, 197, 198, 199, 200, 201, 202, 203, or 204 and the tip still matches the design
...
No task in this change requires Arya's physical iPhone, Apple Watch, microphone, or speaker. An agent MUST NOT claim that a microphone or a speaker was proven. A spoken note is a local binary transcript or a local TTS exit code. Pull request 200 does not prove a speaker played audio. Pull request 202 does not prove a speaker played audio or that a paper was read. Pull request 203 does not prove a speaker or that a paper was read. Pull request 204 does not prove a speaker or that a paper was read. Those device checks are handed back to a human.
...
- **WHEN** an agent is about to claim both jobs are green on `92d776c`
- **THEN** the agent records that daemon CI is green on `92d776c` (check run `35749935885`) while apps (Xcode) is still running
...
- **WHEN** an agent is about to claim both jobs are green on `4d6ddbb` or to claim CI on `fb0aab8`
- **THEN** the agent records that daemon CI is green on `4d6ddbb` (check run `35750995976`) while apps (Xcode) is still running
