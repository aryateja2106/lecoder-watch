...
- **WHEN** an agent re-checks pull request 139, 145, 154, 160, 162, 174, 176, 182, 183, 187, 189, 193, 195, 197, 198, 199, 200, 201, 202, 203, 204, or 205 and the tip still matches the design
...
No task in this change requires Arya's physical iPhone, Apple Watch, microphone, or speaker. An agent MUST NOT claim that a microphone or a speaker was proven. A spoken note is a local binary transcript or a local TTS exit code. Pull request 200 does not prove a speaker played audio. Pull request 202 does not prove a speaker played audio or that a paper was read. Pull request 203 does not prove a speaker or that a paper was read. Pull request 204 does not prove a speaker or that a paper was read. Pull request 205 does not prove a speaker or that a paper was read. Those device checks are handed back to a human.
...
#### Scenario: Daemon-only CI on the saved reply ask is not both jobs green

- **WHEN** an agent is about to claim both jobs are green on `294d252`
- **THEN** the agent records that daemon CI is green on `294d252` (check run `35753000381`) while apps (Xcode) is still running
- **AND** the agent does not claim both jobs are green on `294d252`
