...
- **WHEN** an agent re-checks pull request 139, 145, 154, 160, 162, 174, 176, 182, 183, 187, 189, 193, 195, 197, 198, 199, or 200 and the tip still matches the design
...
No task in this change requires Arya's physical iPhone, Apple Watch, microphone, or speaker. An agent MUST NOT claim that a microphone or a speaker was proven. A spoken note is a local binary transcript or a local TTS exit code. Pull request 200 does not prove a speaker played audio. Those device checks are handed back to a human.
...
#### Scenario: Daemon-only CI on the PDF speak is not both jobs green

- **WHEN** an agent is about to claim both jobs are green on `d4486c4`
- **THEN** the agent records that daemon CI is green on `d4486c4` (check run `35746494919`) while apps (Xcode) is still running
- **AND** the agent does not claim both jobs are green on `d4486c4`
- **AND** the agent records that daemon CI and Xcode CI are both green on handoff tip `2aad3fd` (check run `35746528962`) and on handoff tip `b3e9160` (check run `35745531307`)
