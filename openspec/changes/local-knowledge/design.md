...
- Keep finished drafts as the implementation, including the in-place note replace, the replaced-note draft, the replaced-note hold, the spoken note replace, the spoken-note draft, the spoken replace speak, and the spoken replace draft. Re-check each tip before editing the same files.
...
- A second copy of pull requests 139, 145, 154, 160, 162, 174, 176, 182, 183, 187, 189, or 193.
...
PR 139 is the local PDF note and the knowledge module that owns its own file. PR 145 is spoken-out through `MESH_TTS`. PR 154 is the note-to-held-file path. PR 160 is the route-gated draft. PR 162 is spoken-in through `MESH_STT`. PR 174 is the in-place note replace. PR 176 is the replaced-note draft. PR 182 is the replaced-note hold. PR 183 is the spoken note replace. PR 187 is the spoken-note draft. PR 189 is the spoken replace speak. PR 193 is the spoken replace draft. Pull request 192 (tip `6faf3f3`) already named pull request 189. An agent who needs one of those behaviors opens that pull request and re-checks the tip. Do not start another copy of any of them.
...
- [An agent rebuilds PR 139, 145, 154, 160, 162, 174, 176, 182, 183, 187, 189, or 193] → The tasks point at the existing pull request, record the current `headRefOid`, and stop. A new file for one of those slices is a failed task.
