...
| Seal bodies for the sealed upload | PR 152, branch `cursor/menu-bar-sync-e469` | open draft; the file is not in the Xcode target |
...
- A second copy of the account pages, the identity SQL, the seal, the key file, the note draft, or the Jev filter.
...
PR 135 is the identity SQL. PR 150 is the account pages. PR 152 is the seal. PR 155 and PR 157 are the key file and its Swift reader. PR 154 is the note draft. PR 156 is the Jev filter. An agent who needs one of those behaviors opens that pull request and re-checks the tip.
...
The key-file format is PR 157. Do not start another format. The seal bodies are PR 152. Do not write a second seal.
...
- [An agent rebuilds the pages, SQL, seal, key file, note draft, or filter] → The tasks point at the existing pull request and stop. A new file for one of those slices is a failed task.
